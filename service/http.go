...
func (hs *HttpService) HandlerReload(w http.ResponseWriter, req *http.Request) {
	...
	if reflect.DeepEqual(nfc, hs.cfg) {
		...
		hs.WriteError(w, req, http.StatusInternalServerError, "config not changed")
		return
	}
	...
	if reflect.DeepEqual(nfc, hs.cfg) {
		...
		hs.WriteError(w, req, http.StatusInternalServerError, "config changed, but ignored: listen_addr, idle_timeout, https_enabled, https_cert, https_key")
		return
	}
	...
	hs.WriteText(w, http.StatusOK, nfc.String())
}
...
func (hs *HttpService) HandlerTransferStats(w http.ResponseWriter, req *http.Request) {
	...
	if statsType == "rebalance" || statsType == "recovery" || statsType == "resync" || statsType == "cleanup" {
		hs.Write(w, req, http.StatusOK, hs.tx.CircleState(circleId).Stats)
	} else {
...
func (hs *HttpService) checkAuth(w http.ResponseWriter, req *http.Request) bool {
	...
	q := req.URL.Query()
	if u, p := q.Get("u"), q.Get("p"); hs.compareAuth(u, p, username, password) {
		return true
	}
	if u, p, ok := req.BasicAuth(); ok && hs.compareAuth(u, p, username, password) {
		return true
	}
	if u, p, ok := hs.parseAuth(req); ok && hs.compareAuth(u, p, username, password) {
		return true
	}
...
func (hs *HttpService) compareAuth(u, p, username, password string) bool {
	return hs.transAuth(u) == username && hs.transAuth(p) == password
}
